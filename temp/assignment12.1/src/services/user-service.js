...
module.exports = {
    ...
    getAllUsers: async()=>{
        console.log("user-db",)
        return await sequelize.models.User.findAll({attributes:['name','email','roles']});
    },
    ...
    authorizeFor: (roles)=>{
        ...
        return (request, response, next)=>{
            console.log("request.body.role",request.body);
            console.log("roles",roles);
            if(roles.includes(request.body.roles)){
                next();
            }
            else{
                return new ResponseError("User is not authorized",403);
            }
        }
