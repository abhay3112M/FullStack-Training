...
var search = function(list, match){
    var result=[];
...
var BookManager = function(){
    ...
    this.getBooksByisbn = function(ISBN){
        return search(this.db,book=>(book.isbn===ISBN))
    }

    this.getBooksByAuthor=function(author){
        author=author.toLowerCase();        
        return search(this.db, book=>book.author.toLowerCase().includes(author));
    }
    this.getBooksByTitle=function(title){
        title=title.toLowerCase();       
        return search(this.db, book=>book.title.toLowerCase()===title);
    }

    this.addToTable = function(bookList){
        var tableContent = document.getElementById("tableId");
        tableContent.innerHTML = '<tr><th>Cover</th><th>Title</th><th>Author</th><th>Price</th>><th>Rating</th></tr>';
        for(var eachBook of bookList){
            tableContent.innerHTML += '<tr><td><img src="'+eachBook.cover+'"></td><td>'+eachBook.title+'</td><td>'+eachBook.author+'</td><td>'+eachBook.price+'</td><td>'+eachBook.rating+'</td></tr>'
        }
    }

    this.performFunction = function(){
        var category = document.getElementById("bookType");
        var searchValue = document.getElementById('searchType');
        if(category.selectedIndex <=0 || searchValue.value==="")
          alert("Please Select Valid Category and Related value");
        else if (category.selectedIndex===1){
            var isbn = searchValue.value;
            var listofBooks = this.getBooksByisbn(isbn);
            this.addToTable(listofBooks);
        }
        else if (category.selectedIndex===2){
            var author = searchValue.value;
            var listofBooks = this.getBooksByAuthor(author);
            this.addToTable(listofBooks);
        }
        else{
            var title = searchValue.value;
            var listofBooks = this.getBooksByTitle(title);
            this.addToTable(listofBooks);
        }
    }
};
