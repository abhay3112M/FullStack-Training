...
var app = function() {
    ...
    function populateList(books) {
        ...
        books.forEach(book => {
            ...
            <tr>
                ...
                <td>${book.rating}</td>
                <td>
                <a href="./book-details.html" onclick="obj.showDetails('${encodeURIComponent(JSON.stringify(book))}');">Details</a>
                <a href="./edit-book.html" >Edit</a>
                <a onclick="obj.deleteBook('${encodeURIComponent(JSON.stringify(book))}');">Delete</a>
                </td>
            </tr>
            `;
        });
    ...
    */
    ...
    this.addBook = () =>{
        ...
        var Bcover = document.getElementById('cover').value;
        var Brating = document.getElementById('rating').value;
        var Bdesc = document.getElementById('description').value;
        var Nbook = {
            "title":Btitle,
            "author":Bauthor,
            "price":Bprice,
        ...
        manager.addBook(Nbook);
        alert('Book added successfully');
        open('./addbook.html',"_self");
    }

    this.showDetails = function(book){
        bookObj = JSON.parse(decodeURIComponent(book));
        localStorage.setItem("temp", JSON.stringify(bookObj));
    }

    this.showBookHtml = ()=>{
        var keys = Object.keys(localStorage);
        var i = keys.length;
        lastBook = JSON.parse(localStorage.getItem(keys[i-1]));
        var addto = document.getElementById('container');
        for(var key in lastBook){
            if(key=="_id") continue;
            if(key!="cover"){
                addto.innerHTML += `
                    <h1>${key}</h1>
    ...
    this.deleteBook=(book)=>{
        ...
        for(var i=0;i<books.length;i++){
            if(JSON.stringify(bookObj) === JSON.stringify(books[i])){
